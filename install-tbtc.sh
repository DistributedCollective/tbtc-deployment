...
if [ "$BTC_NETWORK" == "regtest" ]; then
  ...
  npx truffle --network $DEST_NETWORK exec "$WORKDIR/relay-genesis/update-mock-relay.js"
fi
...
./scripts/initialize.sh --network $DEST_NETWORK --application-address $TBTC_SYSTEM_CONTRACT_ADDRESS
